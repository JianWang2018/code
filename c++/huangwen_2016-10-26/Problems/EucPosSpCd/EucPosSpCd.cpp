...
namespace ROPTLIB{
	...
	double EucPosSpCd::f(Variable *x) const
	{
		...
		for (integer k = 0; k < N; k++)
		{
			...
			if (info != 0)
			{
				std::cout << "Warning: Cholesky decompsition in EucPosSpCd::f failed with info:" << info << "!" << std::endl;
			}
		}
		...
		for (integer k = 0; k < N; k++)
		{
			...
			if (info != 0)
			{
				std::cout << "Warning: solving linear system in EucPosSpCd::f failed with info:" << info << "!" << std::endl;
			}
	...
	void EucPosSpCd::EucGrad(Variable *x, Vector *gf) const
	{
		...
		for (integer i = 0; i < N; i++)
		{
			...
			if (info != 0)
			{
				std::cout << "The cholesky decompsotion in EucPosSpCd::EucGrad failed with info:" << info << "!" << std::endl;
			}
	...
	{
		std::cout << "warning: EucPosSpCd::RieHessianEta has not been implemented!" << std::endl;
		etax->CopyTo(xix);
	};
