...
//		return MinPNormConHullMatlab(Mani, x,Ys, LYs, solver, Hv, Soln, YtPY, inc);
		...
		void (QuasiNewton::*Hv)(Vector *v, Vector *result), Vector *Soln, double *YtPY, integer inc)
	{
		...
		for (integer i = 0; i < LYs; i++)
		{
			...
			//std::cout << "i:" << i << ":" << Pnorm2Ys[i] << std::endl;//---
		}
		...
		while (1)
		{
			...
			//std::cout << "xPnorm2:" << result << ", alpha:" << alpha << ", diff:" << alpha - result << std::endl;//---
			...
			for (integer i = 0; i < LYs; i++)
			{
				//std::cout << "i:" << i << ", alpha - yi:" << alpha - XtPYs[i];//--------
				if (fabs(alpha - XtPYs[i]) <= NumErr)
				//if (alpha == XtPYs[i])
					idxYs[i] = true;

				//std::cout << " : " << idxYs[i] << std::endl;//---
			}
			//for (integer i = 0; i < LYs; i++)//-----
			//	std::cout << idxYs[i] << "";//----------
			//std::cout << std::endl;//---
			...
			//std::cout << "yPnorm2:" << result << ", beta:" << beta << ", diff:" << beta - result << std::endl;//---
			...
			for (integer i = 0; i < LYs; i++)
			{
				if (!idxYs[i])
				{
					...
					if (tmp > 0)
					{
						...
						//std::cout << "tmp1:" << tmp << ",tmp2:" << tmp2 << std::endl;//-------
						//std::cout << "i:" << i << ",deno:" << tmp2 << ", nume:" << tmp << ",lambda:" << tmp2 / tmp << ", i:" << i << std::endl;//---
		...
		QuasiNewton *solver, void (QuasiNewton::*Hv)(Vector *v, Vector *result), Vector *Soln, integer &times)
	{
		...
		while (1)
		{
			...
			//std::cout << "sub lambda:" << lambda << std::endl;//--------
			Mani->VectorLinearCombination(x, 1.0 - lambda, X, lambda, Y, X);
		}
		delete[] idxYs;
		delete[] XtPYs;
