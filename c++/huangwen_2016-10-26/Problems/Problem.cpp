...
namespace ROPTLIB{
	...
	{
		EucGrad(x, gf);		
		/*assume intrinsic representation does not requre a projection.*/
		if (!Domain->GetIsIntrinsic() || Domain->GetName() != "Stiefel" || UseHess || (Domain->GetName() == "Stiefel" && x->TempDataExist("Perp")))
		{
