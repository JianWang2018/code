...
int main(void)
{
	...
	std::cout << "random seed:" << tt << std::endl;
	tt = 0;
	//tt = 1463379963;
	//tt = 1462047922;
	genrandseed(tt);
...
void testOrthBoundingBox(double *E, integer d, integer n, double *X, double *Xopt)
{
	...
	RBFGSLPSubsolver->Max_Iteration = 10000;
	RBFGSLPSubsolver->OutputGap = 1;
	//RBFGSLPSubsolver->CheckParams();
	RBFGSLPSubsolver->Run();
	delete RBFGSLPSubsolver;

	//std::cout << "LRBFGSSub" << std::endl;
	//LRBFGSLPSub *LRBFGSLPSubsolver = new LRBFGSLPSub(&Prob, &OrthX);
	...
	//////	dgfs[i]->Print("dgfs");//---
	//////}
	////std::cout << "norm:" << MinPNormConHull(&Domain, const_cast<Variable *>(RBFGSsolver->GetXopt()), dgfs, length, nullptr, nullptr, 0) << std::endl;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	ParseSolverParamsAndOptimizing(prhs[4], &Prob, &OrthX, plhs);
	...
	//std::cout << "length:" << length << ", norm:" << testzero << std::endl;
