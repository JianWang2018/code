...
%         O(:, end) = - O(:, end);
%     end
%     C2plot = C2plot * O; C2plot(:, 2) = C2plot(:, 2) + 0.3;
%     plot(C2plot(:, 1), C2plot(:, 2), '.r-')
%     %%3
%     C2plot = Center_C(C2); C2plot = C2plot / norm(C2plot);
%     O = orth(randn(d, d));
...
%     N=n;
%     idx = (0 : (N - 1)) / (N - 1);
% %     a = 1;b = 5;gammatest = ((idx + a).^b - a^b) / ((1 + a)^b - a^b);
% %     gammatest = ((idx - 0.5).^3 + 0.5^3) .* ((idx - 0.5).^3 - 0.5^3) .* (idx - 0.5) * 10 + idx;
% %     gammatest = ((idx - 0.5).^2 + 0.5^2) .* ((idx - 0.5).^2 - 0.5^2) .* (idx - 0.5) * 8 + idx;
% %     gammatest = idx .* (idx - 0.2) .* (idx - 1) .* (idx - 4 / 3) * 3 + idx;
%     gammatest = sin(idx * 2 * pi) / (4 * pi) + idx; %
%     ppC1 = csape((0:(N-1))/(N-1), C2plot', 'periodic');
%     C2plot = ppval(ppC1, gammatest)';
%     
%     C2plot = C2plot * O; C2plot(:, 1) = C2plot(:, 1) + 0.3;
%     plot(C2plot(:, 1), C2plot(:, 2), '.r-')
%     
%     %%4
%     C2plot = Center_C(C2); C2plot = C2plot / norm(C2plot);
...
%         O(:, end) = - O(:, end);
%     end
%     C2plot = C2plot * O; C2plot(:, 2) = C2plot(:, 2);
%     N=n;
%     idx = (0 : (N - 1)) / (N - 1);
% %     a = 1;b = 5;gammatest = ((idx + a).^b - a^b) / ((1 + a)^b - a^b);
% %     gammatest = ((idx - 0.5).^3 + 0.5^3) .* ((idx - 0.5).^3 - 0.5^3) .* (idx - 0.5) * 10 + idx;
...
%     C2plot = ppval(ppC1, gammatest)';
%     
%     C2plot = C2plot * O; C2plot(:, 1) = C2plot(:, 1) + 0.35;
%     plot(C2plot(:, 1), C2plot(:, 2), '.r-')
%     
%     axis tight;axis equal;axis off;
%     
...
%     subplot(1, 3, 3);
    plot_matching_curves(C1', (C2 * QNO')', gammaQN, 1, '');
...
function plot_matching_curves(C1, C2, gamma, L, titlestr)
    ...
    for i = 1 : N
        ...
        subplot(1,2,1);
        scatter(C1(1, i), C1(2, i), 10, [color_hsv(i, :)]);
        subplot(1,2,2);
        scatter(VV(1), VV(2), 10, [color_hsv(i, :)]);
%         scatter([C1(1, i), VV(1)], [C1(2, i), VV(2)], 10, [color_hsv(i, :)]);
%         plot([C1(1, i), VV(1)], [C1(2, i), VV(2)], 'k');
        if(i == 1)
            subplot(1,2,1);
            scatter(C1(1, i), C1(2, i), '*', 'k');
            subplot(1,2,2);
            scatter(VV(1), VV(2), '*', 'k');
