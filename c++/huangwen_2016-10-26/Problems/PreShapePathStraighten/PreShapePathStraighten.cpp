...
namespace ROPTLIB{
	...
	double PreShapePathStraighten::f(Variable *x) const    //cost function  ?????????????????????
	{
		...
		for (integer t = 0; t < numC; t++)
		{
			...
			{
				//std:: cout << "**************tttttttttttt" << t <<std::endl;
				//ForDebug::Print("pathpathpath", Path_x+t*numP*dim, numP, dim);
				Item_2(Path_x + t*numP*dim, numP, dim, temp + t*numP*dim);
				//ForDebug::Print("temptemptemp++++++++++++", temp+t*numP*dim, numP, dim);
			}
		}
	...
	//    std::cout << tt[1] <<std::endl;
		...
		//ForDebug::Print("wwwwwwwwwwwwwwwwwwwww", w, innumP, indim);
